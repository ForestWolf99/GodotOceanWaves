...
func init_gpu(num_cascades : int) -> void:
	...
	var spectrum_sampler_set := context.create_descriptor_set([descriptors[&'spectrum'].as_sampler()], spectrum_modulate_shader, 0)
	...
	pipelines[&'spectrum_modulate'] = context.create_pipeline([map_size/16, map_size/16, 1], [spectrum_sampler_set, fft_buffer_set], spectrum_modulate_shader)
